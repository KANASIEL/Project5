...
## 주요 크롤링 코드 🕷️ <details><summary>(클릭해서 펼쳐보기 - final_crawler_krx_naver.py 전체 코드)</summary>

**파일명**: `crawler_krx_naver.py`  
**용도**: 네이버 증권에서 KOSPI/KOSDAQ 전 종목 시세를 매일 자동 크롤링 → MongoDB 저장 + Redis 캐시 갱신  
**자동화**: Linux(Ubuntu)에서 crontab을 통한 예약 실행 오전 9시 ~ 오후 15시30분
...
kospi_col = db["naver_kospi"]    # KOSPI 종목 컬렉션
kosdaq_col = db["naver_kosdaq"]  # KOSDAQ 종목 컬렉션

# Redis 연결 (실시간 캐시 무효화용)
try:
...
# MongoDB 인덱스 자동 생성 (code 기준 unique 인덱스)
...
# ================== 데이터 정제 함수 ==================
def clean_int(text):
    """콤마 제거 후 정수 변환, N/A 등은 None 반환"""
    if not text or text.strip() in ["N/A", "-", ""]:
        return None
    return int(text.replace(",", ""))

def clean_float(text):
    """콤마 제거 후 실수 변환, N/A 등은 None 반환"""
    if not text or text.strip() in ["N/A", "-", ""]:
        return None
    ...
    try:
        ...
        for row in rows:
            ...
            data.append({
                ...
                "change": cols[3].get_text(strip=True),          # ±값 문자열 그대로
                "change_rate": cols[4].get_text(strip=True),     # ±퍼센트 문자열
...
def run_crawler():
    ...
    for market_name, sosok, collection in [
        ...
        print(f"\n{market_name} 크롤링 시작...")
        all_items = []
        empty_streak = 0  # 빈 페이지 연속 카운트
        
        for page in range(1, 60):  # 최대 60페이지까지 (실제보다 넉넉히)
            items = crawl_page(sosok, page)
            if not items:
                empty_streak += 1
                if empty_streak >= 3:  # 3페이지 연속 빈 경우 종료
            ...
            time.sleep(random.uniform(0.3, 0.7))  # 서버 부하 방지
        
        # MongoDB에 일괄 업서트 (code 기준 중복 방지)
        if all_items:
    ...
    # Redis 캐시 무효화 (프론트엔드에서 최신 데이터 다시 불러오도록)
