...
public interface StockKoreaDAO extends MongoRepository<StockKoreaDTO, String> {
    // 하루 전 날짜 기준 전체 리스트
    List<StockKoreaDTO> findByBasDt(String basDt);

    // 하루 전 날짜 기준 + 검색어 필터링
    @Query("{ 'basDt': ?0, $or: [ { 'itmsNm': { $regex: ?1, $options: 'i' } }, { 'srtnCd': { $regex: ?1, $options: 'i' } } ] }")
    List<StockKoreaDTO> findByBasDtAndKeyword(String basDt, String keyword);

    // 종목코드로 해당 주식 DB 데이터 전부 조회
    List<StockKoreaDTO> findBySrtnCd(String srtnCd);
