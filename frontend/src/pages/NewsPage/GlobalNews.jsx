...
function GlobalNews() {
  const MEDIA_LOGOS = {
    ...
    "Yahoo Finance":
      "https://s.yimg.com/cv/apiv2/default/logo_yahoo_finance.png",
  };

  const springBaseUrl = "http://localhost:8585";
  const CATEGORIES = ["전체", "Reuters", "CNBC", "CNN", "BBC", "Yahoo Finance"];
  ...
  // 🔍 검색 API 호출 (카테고리 + 키워드)
  const fetchSearchResults = async (query, category = activeCategory) => {
    ...
    try {
      ...
      const url = `${springBaseUrl}/news/global/search?` +
  ...
  const openModal = (news) => {
    const normalized = {
      ...
      mediaLogo: news.mediaLogo || MEDIA_LOGOS[news.source] || null,
      author: news.author || "",
  ...
  const addToRecentlyViewed = (news) => {
    const data = {
      title: news.title,
      media: news.source || news.media,
      _id: news._id,
      link: news.link,
      mediaLogo: news.mediaLogo || null,
      author: news.author || "",
      pubDate: news.pubDate || news.publishedAt || null,
      image_url: news.image_url,
      content: news.content,
    ...
    setRecentlyViewed((prev) => {
      const updated = [data, ...prev.filter((i) => i.title !== data.title)].slice(
        0,
        5
      );
              ...
              <div className="modal-meta">
                <div className="left-meta">
                  {selectedNews.mediaLogo && (
                    <img src={selectedNews.mediaLogo} className="media-logo" alt="" />
                  )}
